...
module Utils =
    ...
    let internal tryCreateRoutineDescriptor (routinesTable: Map<string, System.Type>) (x: RoutineStrategy) =
        ...
        with
        | e ->
            printf $"Error when resolving %s{x.Key}: %s{e.Message}"
            None
    ...
    let configureServices (jobsAssemblies: Assembly []) (hb: HostBuilderContext) (s: IServiceCollection) =
        try
            let config = hb.Configuration
            let ops = WorkerOptions()

            let root = config.GetSection "WorkerOptions"
            ...
            let sleep =
                match tryGetConfigValue uint64 "WorkerOptions:SleepMilliseconds" config with
                | None ->
                    let def = 10000UL
            ...
            let routinesTable =
                jobsAssemblies |> toKeyedRoutinesCorrelations
            ...
            for k, t in routinesTable do
                try
                    ...
                    let schedule = tryGetConfigValue id scheduleName config
                    ...
                    let concurrency =
                        match tryGetConfigValue uint16 concurrencyName config with
                        | None ->
                            let def = 1us
                    ...
                    let timeout =
                        match tryGetConfigValue uint64 timeoutName config with
                        | None ->
                            let def = 60000UL
...
type HostBuilderExtensions() =
    ...
    static member AddPulseeR(builder: IHostBuilder, jobsAssemblies: Assembly []) =
        builder.ConfigureServices(Utils.configureServices jobsAssemblies)
